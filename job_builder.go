...
func (l *CacheLocker) Lock(ctx context.Context, key string) (gocron.Lock, error) {
	client := lockClientWithContext(l.client, ctx)
	locked, err := client.TryLock(l.lockKey(key), l.ttl)
	if err != nil {
		return nil, err
	}
	if !locked {
...
func (l *cacheLock) Unlock(ctx context.Context) error {
	return lockClientWithContext(l.client, ctx).Unlock(l.key)
}

func lockClientWithContext(client cache.LockAPI, ctx context.Context) cache.LockAPI {
	if contextual, ok := any(client).(interface{ WithContext(context.Context) *cache.Cache }); ok {
